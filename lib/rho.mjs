...
// 拉所有 ticker (实时盘口), 返回 { symbol -> ticker } map
export async function fetchAllTickersMap(apiBase, opts = {}) {
    const http = axios.create({ baseURL: apiBase.replace(/\/$/, ""), timeout: 20000, ...opts });
    const res = await http.get("/api/v1/tickers");
    const data = res.data;
    // 兼容多种返回格式: array / { tickers: [...] } / { symbols: [...] }
    ...
    const map = {};
    for (const t of arr) {
        // 单个 ticker 可能是 { ticker: {...} } 嵌套, 或 flat
        const tk = t.ticker ?? t;
        if (tk?.symbol) map[tk.symbol] = tk;
    }
    return map;
}
